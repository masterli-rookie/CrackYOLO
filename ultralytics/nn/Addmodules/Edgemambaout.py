...
class GatedHVMixer(nn.Module):
    """轻量版 GatedHVMixer： 1) 归一化 2) 横/纵方向差分扫描 3) depthwise 扫描增强 4) 通道门控.
    """
    ...
    def __init__(self, dim, num_heads=4, kv_bias=True):
        ...
        self.scan_h = nn.Conv2d(dim, dim, kernel_size=(1, 3), padding=(0, 1), groups=dim, bias=False)
        self.scan_v = nn.Conv2d(dim, dim, kernel_size=(3, 1), padding=(1, 0), groups=dim, bias=False)
...
class HaarDWT(nn.Module):
    """更快的 Haar DWT： - 不用 conv2d - 用 2x2 slicing 直接计算 - 输出 [B, 4C, H/2, W/2].
    """
...
class WaveletEdgeEnhance(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.fuse = nn.Sequential(nn.Conv2d(dim, dim, 1, bias=False), nn.BatchNorm2d(dim), nn.SiLU(inplace=True))

    def forward(self, x):
        coeffs = self.dwt(x)
        _ll, lh, hl, hh = coeffs.chunk(4, dim=1)
...
class EdgeMambaOut(nn.Module):
    """保持类定义不变： - 输入输出通道仍然是 dim -> dim - 构造参数不变 - 仅优化内部实现.
    """

    def __init__(self, dim, k_size=11, num_heads=4, drop_path=0.0):
        super().__init__()
        ...
        # ① 条形卷积：局部边缘
        self.strip_h = nn.Conv2d(dim, dim, kernel_size=(1, k_size), padding=(0, k_size // 2), groups=dim, bias=True)
        self.strip_v = nn.Conv2d(dim, dim, kernel_size=(k_size, 1), padding=(k_size // 2, 0), groups=dim, bias=True)
    ...
    def forward(self, x):
        ...
        # 4) 三路融合
        fused = torch.cat([h_edge, v_edge, edge_mag + self.wavelet_gate * wave_edge + m_out], dim=1)
...
class C3k2_CMamba(nn.Module):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        if c3k:
            self.m = nn.ModuleList(C3k_EdgeMambaOut(c_, c_, 2, shortcut, g) for _ in range(n))
        else:
            self.m = nn.ModuleList(EdgeMambaOut(c_) for _ in range(n))
